...
int main (void) { // Compilar no Linux com gcc BuscaCoordenadas.c -o BuscaCoordenadas -lcurses -lm
	...
	InserePontos(Pontos,Ponto,&raio); // Gera 20 pontos aleat�rios (X dimens�es) de 1 a 40.
	ExibePontos(Pontos); // Exibe os pontos antes da ordena��o (2 dimens�es)
	//InsereArvoreR(&T,Pontos,0,TF,0); // Insere os pontos na �rvore do tipo KD-Tree de maneira Recursiva
	InsereArvore(&T,Pontos); // Insere os pontos na �rvore do tipo KD-Tree de maneira Iterativa
	ExibeArvore(T);
	getch();
