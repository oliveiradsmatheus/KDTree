...
{ // KD
	...
	*raio = rand() % 10 + 1;
	...
	for (i = 0; i < TF; i++)
		for (j = 0; j < K; j++)
			Pontos[i][j] = rand() % 40 + 1; // A fun��o rand() % N gera um n�mero aleat�rio de zero at� o n�mero contido na vari�vel N menos 1
}

void DesenhaPlano(KDTree *raiz, int CI, int LI, int CF, int LF, int N, Pont **Linha, Pont **Coluna)
{ // 2D
	MatEsp *Campo;
	int i;
	...
	if (raiz)
	{
		...
		textcolor(15);
#ifdef __linux__
		gotoxy(48, 4 + 3 * raiz->ponto[0]);
		printw("%d", raiz->ponto[0]);
		gotoxy(48 - (raiz->ponto[1]), 5);
		printw("%d", raiz->ponto[1]);
		refresh();
#else
		...
		textcolor(0);
#ifdef __linux__
		gotoxy(48 - raiz->ponto[1], 4 + 3 * raiz->ponto[0]);
		printw("%s (%d,%d)", "¤", raiz->ponto[0], raiz->ponto[1]);
		refresh();
#else
...
void ExibePlano(KDTree *raiz, int Ponto[K], int CI, int LI, int CF, int LF, int N)
{
	...
	/*system("cls"); // Fun��o de teste para exibi��o da Matriz Esparsa
	ExibeMatriz(Linha);
	getch();*/

	textcolor(12);
#ifdef __linux__
	gotoxy(48 - Ponto[1], CI + Ponto[0] * 3);
	printw("%s (%d,%d)", "¤", Ponto[0], Ponto[1]);
	refresh();
#else
...
#ifdef __linux__
	gotoxy(5, (CI + CF) / 2 - strlen(PLinha) / 2);
	printw("%s", PLinha);
	gotoxy(6, (CI + CF) / 2 - strlen(SLinha) / 2);
	printw("%s", SLinha);
	refresh();
#else
